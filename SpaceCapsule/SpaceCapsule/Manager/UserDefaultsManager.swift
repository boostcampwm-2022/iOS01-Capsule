...
class UserDefaultsManager<T: Codable> {
    ...
    static func loadData(key: UserDefaultsKeys) -> T? {
        guard let loadedData = UserDefaults.standard.object(forKey: key.rawValue) as? Data else { return nil }
        guard let decodedData = try? JSONDecoder().decode(T.self, from: loadedData) else { return nil }
        return decodedData
    }
}
